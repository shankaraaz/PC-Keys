  ...
  if (orders.length === 0) {
    return (
      <div className="max-w-xl mx-auto px-4 py-16 text-center space-y-6 text-[#CBD5E1]" id="empty-orders-view">
        <div className="mx-auto h-16 w-16 bg-[#FFFFFF] rounded-full flex items-center justify-center text-white/30 border border-white/[0.08]">
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10 space-y-8 text-[#CBD5E1]" id="orders-page-layout">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="bg-[#3b82f6]/10 text-[#3b82f6] text-[10px] font-bold px-3 py-1.5 rounded-xl border border-[#3b82f6]/25 flex items-center space-x-1.5 self-start uppercase tracking-widest">
      ...
      <div className="space-y-6" id="orders-list">
        {orders.map((order) => (
          <div
            key={order.id}
            className="bg-[#FFFFFF] rounded-3xl border border-white/[0.08] shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-200 print:shadow-none print:border-none"
            id={`order-container-${order.id}`}
          >
            {/* Header row */}
            <div className="bg-[#FFFFFF] border-b border-white/[0.08] px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 print:bg-white">
              <div className="space-y-1">
                ...
                <p className="text-xs sm:text-sm font-mono font-medium text-white">
                  ID: <span className="text-[#3b82f6] font-semibold">{order.id}</span>
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  className="bg-[#FFFFFF] border border-white/[0.08] hover:bg-white/[0.03] text-white/60 hover:text-white text-[10px] uppercase tracking-widest font-bold py-1.5 px-3 rounded-lg flex items-center space-x-1.5 transition-colors duration-150 cursor-pointer disabled:opacity-50 print:hidden"
                  title="Print Invoice"
                >
                  {printingOrderId === order.id ? (
                    <div className="h-3.5 w-3.5 border-2 border-[#3b82f6] border-t-transparent rounded-full animate-spin" />
                  ) : (
            ...
            <div className="p-6 space-y-6">
                  ...
                  <div key={idx} className="py-4 first:pt-0 last:pb-0 space-y-4">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                      <div>
                        ...
                        <p className="text-[9px] text-[#3b82f6] font-bold uppercase tracking-widest mt-1">Quantity: {item.quantity}</p>
                      </div>
                    ...
                    <div className="space-y-2.5 bg-[#FFFFFF]/40 rounded-2xl p-4.5 border border-white/[0.08]">
                      ...
                      <div className="space-y-2">
                        {item.licenseKeys.map((key, kIdx) => (
                          <div
                            key={kIdx}
                            className="flex items-center justify-between bg-[#EEEEEE] border border-white/[0.08] rounded-xl px-4 py-2.5 font-mono text-xs sm:text-sm font-semibold text-white shadow-sm"
                          >
                            <span className="tracking-wide select-all truncate text-white/90">{key}</span>
                            <button
                              onClick={() => copyToClipboard(key)}
                              className="p-1.5 rounded-lg hover:bg-white/[0.03] text-[#3b82f6] transition-all duration-200 cursor-pointer print:hidden shrink-0 ml-2"
                              title="Copy Key"
                            >
                              {copiedKey === key ? (
              ...
              {/* Total Row */}
              <div className="pt-4 border-t border-white/[0.08] flex items-center justify-between bg-[#FFFFFF] -mx-6 -mb-6 p-6 print:bg-white">
                <div>
